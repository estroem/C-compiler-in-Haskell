...
toRtl (Literal l) nextReg _ = ([LoadLit (nextReg + 1) l], nextReg + 1, emptyScope)
...
toAsmLine (Save name reg size) _ _   = ("mov " ++ (getSizeWord size) ++ " ptr [" ++ name ++ "], " ++ getReg reg, [])
toAsmLine (SaveToPtr reg1 reg2 size) _ _ = ("mov " ++ (getSizeWord size) ++ " ptr [" ++ getReg reg1 ++ "], " ++ getReg reg2, [])
...
toAsmLine (SaveLoc offset reg) _ _   = ("mov [esp" ++ (if offset > 0 then "+" else "") ++ show offset ++ "], " ++ getReg reg, [])
